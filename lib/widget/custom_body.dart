  ...
  if (isError) {
    return Center(
      child: Column(
        ...
        children: [
          const Icon(Icons.error_sharp,color: Colors.red,size: 24,),
          Text(
            errorMessage,
            style: customTextStyle.copyWith(color: Colors.red, fontSize: 18,fontWeight: FontWeight.bold),
          ),
        ],
      ),
    );
...
class CommonBody extends StatelessWidget {
  const CommonBody({super.key, required this.controller,
    this.padding=const EdgeInsets.only(top: 8, left: 8, right: 8), required this.mobile, required this.tablet, required this.desktop});
