...
//x=await api.createLead({})
  ...
  void exportEXCEL() {
    ...
    list_doctor_master.forEach((e) {
      list.add({
        "emp_id": (e.docId ?? '').replaceAll(',', ' '),
        "doc_name": (e.docName ?? '').replaceAll(',', ' '),
        "designation":(e.desig??'').replaceAll(',', ' '),
        "department":(e.deptName??'').replaceAll(',', ' '),
      });
    });
     exportJsonToExcel(list);
  }
}
