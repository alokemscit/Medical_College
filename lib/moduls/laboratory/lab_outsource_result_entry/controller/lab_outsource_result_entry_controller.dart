...
import '../model/lab_model_outsource_result.dart';
import '../model/lab_model_outsource_test_data.dart';

class OutSourceResultEntryController extends GetxController
  ...
  void pickFile() async {
    ...
    if (result != null && result.files.single.bytes != null) {
      var pdfBytes = result.files.single.bytes;
      //print(_pdfBytes);
      showPDF_1(pdfBytes,this);
    }
  }
  ...
  void saveTemplate() async {
    ...
    try {
      ...
      var x = await api.createLead([
        {"p_test_id": selected_mrr.value.testId, "p_delta": deltaJson}
      ], 'outsource_template_save');
      loader.close();
      if (checkJson(x)) {
  ...
  void retriveTemplate() async {
    loader = CustomBusyLoader(context: context);
    loader.show();
    // print(selected_mrr.value.testId);
    try {
      var x = await api.createLead([
        {"tag": "43", "p_test_id": selected_mrr.value.testId}
      ], 'getdata_drs');
      loader.close();
      //  print(x);
      if (checkJson(x)) {
        ModelCommon m = x.map((e) => ModelCommon.fromJson(e)).first;
        var r = jsonDecode(m.name!);
