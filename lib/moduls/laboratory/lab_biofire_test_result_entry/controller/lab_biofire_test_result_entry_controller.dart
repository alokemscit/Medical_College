...
class BiofireResultEntryController extends GetxController with MixInController {
  ...
  void showReport2(String id) async {
    dialog = CustomAwesomeDialog(context: context);
    loader = CustomBusyLoader(context: context);
    loader.show();
    try {
      var y = await api.createLead([
        {"tag": "30", "p_resid": id, "p_rpt": "rpt_bio_report"}
      ], "getssrs_drs_report");
     // print(y);
      if (y == []) {
        loader.close();
...
// str_grp  ,str_att  ,p_testid  ,p_mrrid  ,  p_sampleid  ,p_age  ,p_note  , p_eid  ,p_status
  ...
  void onInit() async {
    ...
    try {
      ...
      if (checkJson(x)) {
        list_mrr_details.addAll(x.map((e) => ModelMrrDetailsList.fromJson(e)));
